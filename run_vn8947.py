...
definition = {
    "survey": {
        ...
        "status": "2", "start_date": "2026-03-17 15:59:42", "end_date": ""
    },
    "questions": [
...
result = Pipeline(PipelineConfig(
    ...
    profile_status   = ["approved"],
    datatable_config = r"C:\Users\PC\OneDrive\DevZone\PyPackages\surveyflow\output_test\VN8947\datatable.json",
    # version        = "v1"     # manual → output_test/VN8947/v1/
    # version not set           # auto   → output_test/VN8947/20260408_131049/
)).run()

print(f"\nrawdata  : {result['rawdata_path']}")
print(f"metadata : {result['metadata_path']}")
print(f"datatable: {result.get('datatable_path', 'not generated')}")
print(f"\nrows: {result['rawdata'].shape[0]} | cols: {result['rawdata'].shape[1]}")
