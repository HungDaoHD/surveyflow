...
if __name__ == "__main__":
    ...
    q_cols_csv  = [c for c in df.columns if c.startswith("q")]
    q_keys_meta = list(meta["questions"].keys())

    print(f"\n{'='*60}")
    print(f"rawdata  : {df.shape[0]} rows x {df.shape[1]} cols  ({len(q_cols_csv)} q-cols)")
    print(f"metadata : {len(q_keys_meta)} questions")
    print(f"Consistent (csv == meta): {q_cols_csv == q_keys_meta}")
    print(f"profile_status: {df['profile_status'].unique().tolist()}")
    ...
    for col in ["task_id", "date_time", "profile_status",
                ...
                "q18", "q20", "q21", "q22", "q42"]:
        v = str(r.get(col, ""))
        print(f"  {col:20s}: {v[:80] if v else '(empty)'}")

    print(f"\n--- metadata sample (q7, q18, q22) ---")
    for label, q in meta["questions"].items():
        if label in ("q7", "q18", "q22"):
            print(f"  {label:8s} | {q['answer_type']:15s} | {q['english_question'][:60]}")
